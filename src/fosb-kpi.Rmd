...
```{r}
bv_met_orcid <- bv_orcid %>% filter(!is.na(orcid))
```
...
Via PURE werd een lijst opgevraagd met alle INBO A1 en A2 publicaties van 2020. Overbodige en GDPR-gerelateerde kolommen werden verwijderd. Voor A1 publicaties was bijna steeds een DOI beschikbaar. Waar een DOI ontbrak werd deze manueel opgevraagd en toegevoegd. Deze bewerkte dataset is [hier]() te vinden. Uit deze lijst met A1 publicaties werden alle DOI's geëxtraheerd en ingegeven in de [Unpaywall Simple Query Tool](https://unpaywall.org/products/simple-query-tool). Deze simple query tool genereert een lijst met de Open Access status van elke DOI. 

```{r}
a1 <- read_excel(path = here::here("data", "INBO_artikels_OA_2020.xlsx"),
...
- Totaal aantal A1 artikels
```{r}
a1_total <- nrow(a1)
a1_total
```

- Totaal aantal A1 artikels OA:
...
a1_OA
```
De A2 publicaties waren zelden gelinkt met een DOI. Voor deze publicaties moest de Open Access status dus worden opgezocht. 

```{r}
a2 <- read_excel(path = here::here("data", "INBO_artikels_OA_2020.xlsx"),
                            sheet = "onderzoeksoutput_2020_A2")
```
...
```{r}
(a1_OA + a2_OA)/(a1_total + a2_total) * 100
```
